...
import { renderMaps, toggleBtnsParametr, getInputValues } from "./helpers.js";
...
export function modalsHandler(eventTarget, Game) {
  ...
  if (btnHelp) {
    ...
    setTimeout(() => (btnHelp.disabled = false), 3000);
    return;
  }
  ...
  if (btnWinner) {
    ...
    Game.Screen.screenFinal.render(
      Game.location.current,
      Game.players.getFilteredPlayersBySpyRole()
    );
    document
