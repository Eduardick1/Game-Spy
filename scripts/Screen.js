...
import { setMaxPlayers, insertTextContent } from "./helpers.js";
...
class ScreenPrepare extends Screen {
  ...
  handleEvents(eventTarget, Game) {
    if (eventTarget.closest(this.children.listPlayers))
      ...
      this.rootElement
        ...
        .querySelectorAll(`.${CLASSNAMES.playerElementToDelete}`)
        .forEach((li) => li.classList.remove(CLASSNAMES.playerElementToDelete));
    if (
  ...
  playersListHandler(node, Game) {
    ...
    if (node.closest("button")) {
      liElem.classList.add(CLASSNAMES.deleting);
      liElem.addEventListener(
        "animationend",
        () => {
          Game.players.deletePlayer(liElem.dataset.player);
          liElem.remove();
          this.toggleClassMainBtn(Game.players.length);
          setMaxPlayers(Game.players.length);
    ...
    liElem.classList.toggle(CLASSNAMES.playerElementToDelete);
  }
}
class ScreenStart extends Screen {
  ...
  mainBtnHandler(Game) {
    ...
    if (isCardWithClass) {
      ...
      // btn.textContent = "Посмотреть роль";
